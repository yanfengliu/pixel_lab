...
/**
 * `docs/learning/lessons.md` is read at session start and is short by construction; the
 * context, rule, and pointer live beside it in `lessons-evidence.md`. The split only works
 * if every rule can reach its evidence and no evidence is stranded — a rule whose entry has
 * gone missing is indistinguishable from one that was never proved, and an entry no rule
