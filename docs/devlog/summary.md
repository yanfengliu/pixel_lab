...
foundation of the pixel-drawing feature (data-model migration v1 → v2,
drawing primitives, tool/color/undo store, five new React components,
tool-dispatched Canvas). 156/156 tests pass, `npx tsc --noEmit` clean,
`npx vite build` green (~140 kB gz).
...
- `src/core/` — DOM-free domain: types (now with `SequenceSlicing`,
  ...
  `computeDelta` / `undoDelta` / `redoDelta`.
...
- `src/ui/` — Zustand store with drawing state (activeTool,
  ...
  undo/redo stacks) plus `createBlankSource`, `beginStroke`, `undo`,
  `redo`, swatch actions. Shell is a 5-zone grid with a left rail
  (ToolPalette + ColorPanel), SourcesPanel, tool-dispatched Canvas,
  AnimationsPanel, and the frames zone (FramesStrip + PreviewBar).
  `NewBlankSource` modal dialog wired to TopBar. `usePlayback` hook
  shared between PreviewBar and any future animation consumer.
...
- Phase 2 (line/rect/ellipse/marquee/move) + Phase 3 (onion skin,
  pixel grid, slice-rect tool) of the pixel-drawing plan.
